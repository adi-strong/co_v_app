...
import useSetAgentImageData from "../model/useSetAgentImageData.ts";
import type {AgentImage} from "../model/agentService.ts";
import {initAgentImageState} from "../model/agentService.ts";
import useUploadNewAgentProfile from "../model/useUploadNewAgentProfile.ts";
import RemoveAgentProfileModal from "./RemoveAgentProfileModal.tsx";

export default function UniqueAgentImageForm({ agent, loader, onSubmit, onRefresh, isLoading }: {
  agent: Patient
  loader: boolean
  onRefresh: () => void
  isLoading: boolean
  ...
  const [state, setState] = useState<AgentImage>(initAgentImageState())
  
  const { profil } = agent
  
  useSetAgentImageData(agent, setState)
  useUploadNewAgentProfile(state, setState, onRefresh)
