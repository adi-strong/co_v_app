...
import BonEntreeItem from "./BonEntreeItem.tsx";
import PostNewEntryModal from "./PostNewEntryModal.tsx";

export default function BonEntreeData(props: {
  entries: BonEntree[]
  onRefresh: () => void
  loader: boolean
  isFetching: boolean
    ...
    <>
      ...
      <Table hover responsive>
        ...
        <tr>
          ...
          <th style={{ fontSize: '1rem' }}>Porteur</th>
        ...
        {entries.length > 0 && entries.map((c, index: number) =>
          <BonEntreeItem key={index} entry={c} />)}
        </tbody>
      </Table>
      
      <PostNewEntryModal show={show} onHide={(): void => handleShow(setShow)} onRefresh={onRefresh} />
