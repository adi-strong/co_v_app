...
import {useGetUniqueBonEntreeQuery} from "../model/bonEntree.api.slice.ts";
import {Button, Card, Col, Row, Spinner} from "react-bootstrap";
import LogoLoader from "../../../../loaders/LogoLoader.tsx";
import PrintableContent from "../../../../components/PrintableContent.tsx";
import UniqueBonEntreesDetails from "./UniqueBonEntreesDetails.tsx";

const ShowBonEntree = () => {
  
  useDocumentTitle('Bon des entrées')
  useActivePage('finances')
  
  const contentRef = useRef<HTMLDivElement | null>(null)
  
  const { id } = useParams()
  const { data, isLoading, isFetching, isError, refetch } = useGetUniqueBonEntreeQuery(id)
    ...
    <BodyContainer>
      ...
      <Card>
        <PrintableContent ref={contentRef} className='pt-8 px-7 pe-7'>
          {!(isError && isLoading) && data && <UniqueBonEntreesDetails entry={data}/>}
        </PrintableContent>
...
export default memo(ShowBonEntree)
