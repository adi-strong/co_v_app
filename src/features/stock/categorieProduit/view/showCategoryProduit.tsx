...
import {useGetUniqueCategorieProduitQuery} from "../model/categorieProduit.api.slice.ts";
...
import ProduitsByCategoryList from "./ProduitsByCategoryList.tsx";

const ShowCategoryProduit = () => {
  ...
  const { id } = useParams()
  const { data, isLoading, isFetching, isError, refetch } = useGetUniqueCategorieProduitQuery(id)
  
  const onRefresh = async (): Promise<void> => { await refetch() }
  
  return (
    <BodyContainer>
      <PageTitles title='Catégorie de produits'/>
      ...
      {!(isLoading && isError) && data && (
        <>
          ...
          <ProduitsByCategoryList category={data} />
        </>
      )}
...
export default memo(ShowCategoryProduit)
